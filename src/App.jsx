...
// Robust cross-browser Haptic vibration trigger
const triggerHaptic = (type, enabled = true) => {
  ...
  try {
    ...
    else if (type === 'delete') navigator.vibrate(28);
    else if (type === 'submit') navigator.vibrate([35, 45, 35]);
    else if (type === 'win') navigator.vibrate([50, 60, 50, 60, 90]);
    else if (type === 'error') navigator.vibrate([45, 55, 45]);
  } catch (e) {}
};
...
export default function App() {
  ...
  const [cbType, setCbType] = useState('deuteranopia'); // 'protanopia' | 'deuteranopia' | 'tritanopia' | 'achromatopsia'
  ...
  // 'mode1': Target (Right) vs Guess Slices (Left)
  // 'mode2': Target (Right) vs Guess Blended Color (Left)
  // 'mode3': Guess Blended Color (Right) vs Guess Slices (Left)
  ...
  // Inspection & Active Guess Calculation for 3 Inspection Modes
  ...
  return (
    ...
    >
      {/* Hidden SVG Colorblind Simulation Matrix Filters */}
      ...
      {/* --- HEADER --- */}
      <header className="w-full mx-auto px-4 py-2 flex items-center justify-between border-b border-slate-800/80 bg-[#12131C]/90 backdrop-blur-md flex-shrink-0 z-30">
      ...
      {/* --- FLUID VIEWPORT MAIN BODY (ADAPTS FROM 1:5 TO 5:1) --- */}
      <main className="w-full flex-1 min-h-0 px-3 py-1 flex flex-col lg:flex-row items-center justify-evenly gap-3 lg:gap-8 overflow-hidden">
        
        {/* --- LEFT PANEL: DYNAMIC PIE WHEEL DISPLAY --- */}
        <div className="flex flex-col items-center justify-center gap-1.5 lg:w-1/2 flex-shrink-0 h-full min-h-0">
          <div className="p-3 sm:p-5 rounded-3xl bg-slate-900/60 border border-slate-800/80 shadow-2xl backdrop-blur-md flex flex-col items-center max-h-full">
            <div 
              className={'w-36 h-36 sm:w-48 sm:h-48 lg:w-[28vw] lg:h-[28vw] max-w-[45vh] max-h-[45vh] rounded-full border-4 border-slate-700/80 shadow-2xl relative overflow-hidden transition-all duration-700 transform ' + (
            ...
            >
              {gameStatus === 'won' ? (
                ...
                <svg className="w-full h-full" viewBox="0 0 200 200">
                  ...
                  {/* LEFT HALF (CLIPPED TO X <= 100): Zero Center Line Bleed */}
            ...
            {/* Inspection Controls with 3 Comparison Modes */}
            {inspectingGuessIndex !== null ? (
              <div className="mt-2.5 flex flex-col items-center gap-1.5 animate-fade-in">
                ...
                <button
                  ...
                  className="px-3 py-0.5 rounded-full bg-slate-800 text-slate-300 text-[10px] font-bold hover:bg-slate-700 transition"
                >
                  ✕ Close Comparison
                </button>
              </div>
        ...
        <div className="w-full lg:w-1/2 flex-1 min-h-0 flex flex-col justify-between gap-2 overflow-hidden h-full">
          
          {/* GUESS HISTORY GRID (SCROLLABLE CONTAINER) */}
          <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain touch-pan-y pr-1 custom-scrollbar space-y-1.5">
            {Array.from({ length: maxAttempts }).map((_, rowIndex) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Percentage Badge: Tap to compare side-by-side in 3 modes! */}
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-5 flex flex-col gap-3 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            {/* Colorblind Adjustments (Type Selector + Multi-Select Overlay Flags) */}
