...
export default function App() {
  ...
  // Inspection State (Clicking a historical guess to preview its pie side-by-side)
  ...
  // Active or Inspected Recipe for the Left Pie Half
  const activeLeftRecipe = useMemo(() => {
    if (inspectingGuessIndex !== null && guesses[inspectingGuessIndex]) {
      return guesses[inspectingGuessIndex].recipe;
    }
    ...
    return [];
  }, [inspectingGuessIndex, guesses, currentGuess]);

  /* --- SVG LEFT HALF PIE SLICES (STRICTLY CLIPPED AT X=100) ---
     Uses SVG clipPath so stroke & fill NEVER bleed past the vertical divider!
  */
  ...
  // Keyboard status for disabling and crossing out wrong keys
  ...
  return (
    <div className="h-screen h-dvh max-h-screen w-full bg-[#12131C] text-slate-100 flex flex-col justify-between font-sans selection:bg-purple-500 selection:text-white overflow-hidden relative select-none">
      ...
      {/* --- HEADER --- */}
      <header className="w-full max-w-7xl 2xl:max-w-[1500px] mx-auto px-4 py-2 flex items-center justify-between border-b border-slate-800/80 bg-[#12131C]/90 backdrop-blur-md flex-shrink-0 z-30">
        ...
        <div className="text-center">
          <h1 className="text-xl sm:text-2xl font-black tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-slate-100 via-purple-300 to-slate-200">
            Colorfle
          </h1>
      ...
      <main className="w-full max-w-7xl 2xl:max-w-[1500px] mx-auto flex-1 min-h-0 px-3 py-1 flex flex-col lg:flex-row items-center justify-center gap-3 lg:gap-8 overflow-hidden">
        
        {/* --- LEFT PANEL: PIE WHEEL DISPLAY --- */}
        <div className="flex flex-col items-center justify-center gap-1.5 lg:w-1/2 flex-shrink-0">
          <div className="p-3 sm:p-5 rounded-3xl bg-slate-900/60 border border-slate-800/80 shadow-2xl backdrop-blur-md flex flex-col items-center">
            <div 
              className={`w-36 h-36 sm:w-44 sm:h-44 xl:w-60 xl:h-60 rounded-full border-4 border-slate-700/80 shadow-2xl relative overflow-hidden transition-all duration-700 transform ${
            ...
            >
              {gameStatus === 'won' ? (
                ...
                <svg className="w-full h-full" viewBox="0 0 200 200">
                  ...
                  {/* RIGHT HALF (CLIPPED TO X >= 100): Target Solid Mixed Color ONLY */}
                  ...
                  {/* LEFT HALF (CLIPPED TO X <= 100): Prevents any center line bleed! */}
            ...
            {/* Inspection / Live Preview State Indicator Pill */}
            {inspectingGuessIndex !== null ? (
              <button
                onClick={() => setInspectingGuessIndex(null)}
                className="mt-2 px-3 py-1 rounded-full bg-purple-600/90 text-white text-[11px] font-bold shadow-md hover:bg-purple-500 transition flex items-center gap-1.5"
              >
                <span>Comparing Guess #{inspectingGuessIndex + 1}</span>
                <span className="bg-purple-800 px-1.5 py-0.2 rounded-full text-[9px]">✕ Reset</span>
              </button>
            ) : (
              <div className="text-[11px] font-semibold text-slate-400 flex items-center gap-3 mt-2">
        ...
        <div className="w-full lg:w-1/2 flex-1 min-h-0 flex flex-col justify-between gap-2 overflow-hidden">
          
          {/* GUESS HISTORY GRID (ONLY SCROLLABLE ELEMENT) */}
            ...
            {Array.from({ length: maxAttempts }).map((_, rowIndex) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Percentage Badge: Tap to compare side-by-side in Pie Wheel! */}
                  <div className="w-14 flex items-center justify-end">
                    {guess ? (
                      <button
                        onClick={() => setInspectingGuessIndex(isInspected ? null : rowIndex)}
          ...
          <div className="w-full flex flex-col gap-1.5 flex-shrink-0">
            <div className="grid grid-cols-7 sm:grid-cols-10 gap-1 sm:gap-1.5 bg-slate-900/90 p-2 rounded-2xl border border-slate-800 shadow-inner">
              {PALETTE.map((color) => {
                ...
                return (
                  <button
                    ...
                    className={`h-8 sm:h-9 rounded-lg flex flex-col items-center justify-center relative overflow-hidden transition-all duration-150 transform border-2 border-slate-600/90 active:scale-90 shadow-sm ${
                      isAbsent ? 'cursor-not-allowed opacity-80' : 'hover:scale-105 hover:border-slate-300'
                    } ${badgeStyle}`}
                    style={{ backgroundColor: color.hex }}
                    title={color.name}
                  >
                    {/* Color code text overlay for accessibility */}
                    {cbColorNames && (
                      <span className={`text-[8px] font-black uppercase leading-none drop-shadow ${getContrastTextColor(color.hex)}`}>
                        {color.code}
                      </span>
                    )}
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className="py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs transition disabled:opacity-40"
              >
                Clear
              </button>
              ...
              <button
                ...
                disabled={gameStatus !== 'playing' || currentGuess.length === 0}
                className="py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-rose-300 font-bold text-xs transition disabled:opacity-40 flex items-center justify-center gap-1"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                className="py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs uppercase tracking-wider shadow-md transition active:scale-95 disabled:opacity-40"
              >
                ENTER
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-5 flex flex-col gap-3 shadow-2xl relative">
            ...
            <div className="space-y-2 text-xs text-slate-300 leading-relaxed">
              ...
              <p>Left pie slices run from top (largest contribution e.g. 50%) to bottom (smallest e.g. 15%). Click any previous guess circle to compare side-by-side!</p>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-5 flex flex-col gap-3 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            {/* Colorblind Adjustments (Multiple Active Options Allowed!) */}
            <div className="space-y-1.5 bg-slate-800/60 p-2.5 rounded-xl border border-slate-700">
              <label className="text-xs font-bold uppercase text-slate-300 block">Colorblind Assistance Options</label>
